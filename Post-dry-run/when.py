...
def answering_when(cleansedQuestion,stop_words_free_question,complete_sentence_list,sentence_list,dateline,month_list,time_list):
    ...
    for i in range(0,len(sentence_list)):
        ...
        if time_list[i] != [] or month_list[i]!= []: # Sentence contains a time expression
            ...
            #candidate_sent_list.append(complete_sentence_list[i])
        ...
        for word in cleansedQuestion.split():
            ...
            elif word.lower() in ['start','begin']:
                for sent in  sentence_list[i].split():
    ...
    if max_score_value > dateline_score:
        ...
        if s_timelist != []:
            ...
            for j in range(0, len(temp_list)):
                if temp_list[j] in s_timelist and j!=0:
                    if temp_list[j-1] not in stanford_stop_words_list:
                        answer_list.append(temp_list[j-1])      #Appending the word before the time list which is generally a number or indicative of the time
                    else:
                        answer_list.append(temp_list[j+1])      #Appending the word after the time list word which will be the result in few cases
            ...
            time_prep=['over','period','within','inside','under','ago']
